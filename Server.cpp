/*
TODO: 
[1. WIP] ServerLog file has extra, double, new lines
            - revision (a) -> commented out the "new line maker" in ReceiveMessage()
            - unfixed issue -> there seems to be a single random new line generated by Automobile.cpp log data

[2. FIN] DumpLogData() does not await for keystroke before entering back to the menu
            - revision (a) -> added utility function ClearInputBuffer() to clear std::cin

[2. WIP] to fix SIGINT not working properly
            - revision (a) -> put user inputs into a separate thread so std::cin doesn't block things
            - does not fix the issue
            - speculation: std::cin seems to be blocking the SIGINT 

[3. WIP] SetLogLevel() is bugged -> Logger.cpp side is not updating
